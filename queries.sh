...
echo "$($PSQL "SELECT wt.name FROM games AS wg JOIN teams AS wt ON wg.winner_id = wt.team_id WHERE round = 'Eighth-Final' AND year = 2014 UNION
SELECT ot.name FROM games AS og JOIN teams AS ot ON og.opponent_id = ot.team_id
